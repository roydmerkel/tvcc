...
/* OP 60: PUSHA */
static
unsigned op_60 (e8087_t *c)
{
	unsigned short tmp;

	tmp = e87_get_sp (c);

	e87_push (c, e87_get_ax (c));
	e87_push (c, e87_get_cx (c));
	e87_push (c, e87_get_dx (c));
	e87_push (c, e87_get_bx (c));
	e87_push (c, tmp);
	e87_push (c, e87_get_bp (c));
	e87_push (c, e87_get_si (c));
	e87_push (c, e87_get_di (c));
...
/* OP 61: POPA */
static
unsigned op_61 (e8087_t *c)
{
	e87_set_di (c, e87_pop (c));
	e87_set_si (c, e87_pop (c));
	e87_set_bp (c, e87_pop (c));
	e87_pop (c);
	e87_set_bx (c, e87_pop (c));
	e87_set_dx (c, e87_pop (c));
	e87_set_cx (c, e87_pop (c));
	e87_set_ax (c, e87_pop (c));
...
unsigned op_62 (e8087_t *c)
{
	...
	lo = e87_get_mem16 (c, seg, ofs);
	hi = e87_get_mem16 (c, seg, ofs + 2);
...
/* OP 68: PUSH imm16 */
static
unsigned op_68 (e8087_t *c)
{
	e87_push (c, e87_mk_uint16 (c->pq[1], c->pq[2]));
	e87_set_clk (c, 3);
...
unsigned op_69 (e8087_t *c)
{
	...
	e87_set_reg16 (c, (c->pq[1] >> 3) & 7, d & 0xffff);

	d &= 0xffff8000;
	e87_set_f (c, E87_FLG_C | E87_FLG_O, (d != 0xffff8000) && (d != 0x00000000));
	e87_set_f (c, E87_FLG_Z, d == 0);

	e87_set_clk_ea (c, (22 + 25) / 2, (29 + 32) / 2);
...
/* OP 6A: PUSH imm8 */
static
unsigned op_6a (e8087_t *c)
{
	e87_push (c, e87_mk_sint16 (c->pq[1]));
	e87_set_clk (c, 3);
...
unsigned op_6c (e8087_t *c)
{
	...
	if (c->prefix & (E87_PREFIX_REP | E87_PREFIX_REPN)) {
		while (e87_get_cx (c) > 0) {
			e87_set_mem8 (c, e87_get_es (c), e87_get_di (c), e87_get_prt8 (c, e87_get_dx (c)));
...
unsigned op_6d (e8087_t *c)
{
	...
	if (c->prefix & (E87_PREFIX_REP | E87_PREFIX_REPN)) {
		while (e87_get_cx (c) > 0) {
			e87_set_mem16 (c, e87_get_es (c), e87_get_di (c), e87_get_prt16 (c, e87_get_dx (c)));
...
unsigned op_6e (e8087_t *c)
{
	...
	if (c->prefix & (E87_PREFIX_REP | E87_PREFIX_REPN)) {
		while (e87_get_cx (c) > 0) {
			e87_set_prt8 (c, e87_get_dx (c), e87_get_mem8 (c, seg, e87_get_si (c)));
...
unsigned op_6f (e8087_t *c)
{
	...
	if (c->prefix & (E87_PREFIX_REP | E87_PREFIX_REPN)) {
		while (e87_get_cx (c) > 0) {
			e87_set_prt16 (c, e87_get_dx (c), e87_get_mem16 (c, seg, e87_get_si (c)));
			e87_set_si (c, e87_get_si (c) + inc);
			e87_set_cx (c, e87_get_cx (c) - 1);
			c->instructions += 1;
			e87_set_clk (c, 8);
...
unsigned op_c0 (e8087_t *c)
{
	...
	switch (xop) {
		case 0: /* ROL r/m8, imm8 */
			d = (s << (cnt & 7)) | (s >> (8 - (cnt & 7)));
			e87_set_cf (c, d & 1);
			e87_set_of (c, ((s << 1) ^ s) & 0x80);
			break;

		case 1: /* ROR r/m8, imm8 */
			d = (s >> (cnt & 7)) | (s << (8 - (cnt & 7)));
			e87_set_cf (c, d & 0x80);
			e87_set_of (c, ((d << 1) ^ s) & 0x80);
			break;
...
unsigned op_c1 (e8087_t *c)
{
	...
	switch (xop) {
		case 0: /* ROL r/m16, imm8 */
			...
			e87_set_cf (c, d & 1);
			e87_set_of (c, ((s << 1) ^ s) & 0x8000);
			break;

		case 1: /* ROR r/m16, imm8 */
			d = (s >> (cnt & 15)) | (s << (16 - (cnt & 15)));
			e87_set_cf (c, d & 0x8000);
			e87_set_of (c, ((d << 1) ^ s) & 0x8000);
			break;
...
unsigned op_c8 (e8087_t *c)
{
	...
	if (level > 0) {
		...
		for (i = 1; i < level; i++) {
			bp -= 2;
			e87_push (c, e87_get_mem16 (c, sp, bp));
		}
	...
	e87_set_bp (c, tmp);
	e87_set_sp (c, tmp - e87_mk_uint16 (c->pq[1], c->pq[2]));
...
/* OP C9: LEAVE */
static
unsigned op_c9 (e8087_t *c)
{
	e87_set_sp (c, e87_get_bp (c));
	e87_set_bp (c, e87_pop (c));
	e87_set_clk (c, 8);
